...
% \cite{DBLP:conf/pods/Khamis0PSW22} gives a full characterization of the 
% POPS that guarantee that every \datalogo program terminates.  
...
% independent interest.  With some abuse, we will say in this chapter that
...
% Sec.~\ref{sec:datalogo:conclusions}.
...
atom $R(\bm u)$.
Recall that $V$ is the set of key variables,
 and we define a {\em valuation} to be a function $\theta : V \rightarrow D_0$.
...
In most of the rest of the chapter we will study properties of vector-valued
...
\item[Representing Undefined] The {\em lifted POPS} is
  ...
  $0 \otimes \bot \neq 0$.  
  % Its core semiring is the trivial semiring
  % $S_\bot \oplus \bot = \set{\bot}$.
...
\item[Representing Contradiction] The {\em completed POPS} is
  ...
  % As before, its core semiring is the trivial semiring
  % $S_\bot^\top \oplus \bot = \set{\bot}$.
...
We illustrate two examples of semirings that are {\em stable}, a
...
% In this dissertation we are interested in characterizing the POPS that ensure
...
% In this dissertation we consider an equivalent semantics of \datalogo, which
...
% % }
% % \dan{we say early that all POPS are assumed to be strict in this dissertation}
% % %
...
% diverges.
% We do not use $\omega$-continuity in this dissertation,
...
% this dissertation.  We give next a necessary and sufficient condition for
...
The current definition of the delta rule contains
...
The bridge helps enlarge the set of problems $\name$ can express in a
...
This chapter established part of the bridge. There are many interesting problems left
