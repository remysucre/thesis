...
\begin{align*}
 \begin{array}{l|c|c|c|c|} \cline{2-5}
   ...
   \delta^{(2)} & 0 & 1 & 1 & 1  \\ \cline{2-5}
   T^{(2)} & 1 & 2 & 2 & 2  \\ \cline{2-5}
   \delta^{(3)} & 0 & 0 & 1 & 1  \\ \cline{2-5}
   T^{(3)} & 1 & 2 & 3 & 3  \\ \cline{2-5}
   \delta^{(4)} & 0 & 0 & 0 & 1  \\ \cline{2-5}
   T^{(4)} & 1 & 2 & 3 & 4  \\ \cline{2-5}
   \delta^{(5)} & 0 & 0 & 0 & 0 \\ \cline{2-5}
   \end{array}
\end{align*}
...
If $0 = \bot$, does the POPS have to be naturally ordered?

A POPS is {\em sparse} if $\bot = 0$.

Every naturally ordered POPS is sparse, but the converse is not true.
For example, $S = \setof{ m + n \sqrt{2}}{ m, n \in \N }$.
Then $(S, +, \times, 0, 1)$ with the usual ordering is sparse.
$1 \leq \sqrt{2}$, but $\not \exists x \in S : 1 + x = \sqrt{2}$.
...
\begin{thm}
  Any POPS where $\bot = 0$ satisfies: if $x + z = y$ then $x \leq y$.
\end{thm}
\begin{proof}
First direction: if $x + z = y$ then $x \leq y$.
$x = x + 0 = x + \bot \leq x + z = y$.

Second direction: if $x \leq y$ then $\exists z : x + z = y$.
...
\remy{Prop. 2.4: semiring change to pre-semiring.}
