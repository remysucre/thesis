...
%   Also, the contributions need to align with the text in the chapter.}
% \rw{Fixed.}
...
We chose DuckDB because it is the only system able to produce query plans in a machine-readable format.
...
A \FJ plan may join on any number of variables and relations at each step,
  ...
  \emph{subatom} and \emph{partitioning}.  
...
\begin{defn}[\FJ Plan]
  ...
  $[\phi_1, \ldots, \phi_m]$, where each $\phi_k$, called a {\em node}, is a list of
  subatoms of $Q$.  The nodes are required to
...
\section{Experiments}\label{sec:fj:eval}
